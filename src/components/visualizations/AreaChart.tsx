...
export default function AreaChart({
  ...
  useEffect(() => {
    ...
    const allPoints = data.flatMap(series => series.points.filter(p => p.value !== null));
    ...
    data.forEach(series => {
      const chartData = series.points
        .map(d => ({ ...d, date: parseDate(d.date) as Date, value: d.value ?? 0 }))
      ...
      svg
        ...
        .attr(
          'd',
          d3.area<{ date: Date; value: number | null }>()
